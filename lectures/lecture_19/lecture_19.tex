...
An example would be 
\[
C_i \text{  disjoint  }\implies \mu\left(\bigcup^\infty_{i=1}C_i\right)=\sum_{i=1}^{\infty}C_i
\]
...
\[
\mu\left(T^{-1}c\right)=\mu\left(c\right)
\] 
...
A measure-preserving map is called an endomorphism. If $T$ is invertible, then $T^{-1}$ is also measure preserving and $T$ is called an automorphism.
\[
...
For the right-hand side, since $f(Tx) = 1_C(Tx) = 1 \text{ iff } Tx \in C$,
