...
\]
\underline{Theore}: If $T$ is ergodic, then any invariant function is constant on a set of full measure.
...
Uf(x) = f(Tx)
\]
where $T$ is measure preserving, which implies $\left|\left| Uf \right|\right| = \left|\left| f \right|\right|$.
...
For $f$ constant functions, we have 
\[
Uf = f\implies f 
\]
is an eigenfunction of $U$ with eigenvalue $1$.

If $T$ is ergodic, then constant functions are the only eigenfunctions with eigenvalue of 1. This follows from the definition of ergodicity.

If $Uf=e^{i\theta}f$ and $\theta\neq 0$, then $\int f\, d\mu=0$. For a proof, $f$ is orthogonal to the constant function. For an alternative proof, we can write 
\[
\int_M Uf\, d\mu = \int f(Tx) \, d\mu(x) = \int_M e^{i\theta}f(x) \, d\mu
\]
where because of measure preserving, it follows that 
\[
\int f(x) \, d\mu(x) = \int_M e^{i\theta}f(x) \, d\mu \implies (1-e^{i\theta})\int f(x) d\mu(x) = 0
\]
since $(1-e^{i\theta})=0$.
